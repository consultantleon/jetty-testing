...
public class AuthFilter implements ClientRequestFilter, ClientResponseFilter {
    ...
    private final ClientManager authClientManager;
    private final ClientManager repeatClientManager;

    public AuthFilter(ClientManager authClientManager, ClientManager repeatClientManager) {
        this.authClientManager = authClientManager;
        // when the request.getClient() is used in repeatRequest, processing hangs completely
        // so as a workaround I started testing with a separate client for the repeat requests,
        // that runs stable except for the remaining random earlyEOF exceptions

        // Use the separate repeat client from provided additional repeatClientManager
                this.repeatClientManager = repeatClientManager;

        // or set repeatClientManager to null to use request.getClient() -> this will cause
        // a lockup in processing...! (but just to prove the case).
//        this.repeatClientManager = null;
    }
    ...
    private Response doLoginRequest() {
        ...
        final WebTarget target = authClientManager.getClient().target(TOKEN_URL);
    ...
    public void repeatRequest(ClientRequestContext requestContext,
            ClientResponseContext responseContext) {

        LOG.info("repeatRequest");

        final Client repeatClient = repeatClientManager == null ? requestContext.getClient()
                                                                : repeatClientManager.getClient();
        final WebTarget target = repeatClient.target(requestContext.getUri());
        ...
        if (requestContext.hasEntity()) {
            responseFuture = builder
                    .async()
                    .method(requestContext.getMethod(), Entity.entity(requestContext.getEntity(),
                            requestContext.getMediaType()));
        } else {
            responseFuture = builder.async().method(requestContext.getMethod());
        }
        try {
            final Response repeatResponse = responseFuture.get();
            LOG.info("request repeated, status={}", repeatResponse.getStatus());
            copyResponseToResponseContext(repeatResponse, responseContext);
        } catch (Exception e) {
            throw new WebApplicationException("Failed to repeat the request", e);
